...
export class VectorStorageService {
  ...
  public async uploadFile(
    ...
    const vectorStore = this.qdrantService.getVectorStore(
      storage.qdrantCollectionName,
    );
  ...
  public async getAllStorages(): Promise<StorageDto[]> {
    try {
      const storages: StorageDto[] = await this.storageModel
        .aggregate([
          {
            $lookup: {
              from: 'vector_item',
  ...
  public async getFileItems(storageId: string): Promise<Item[]> {
    try {
      return this.itemModel
        .find({ storageId: new Types.ObjectId(storageId) })
        .exec();
    } catch (error) {
      throw error;
    }
